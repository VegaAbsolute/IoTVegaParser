...
- `time` - reading time for values in this packet in Unix-time format (sec), data type `Number`;
- `timeStringISO` - reading time for values in this packet in ISO format, data type `String`;
- `energyConsumed` - energy consumed (Gcal), data type `Number`;
- `totalVolumeCoolant` - total volume of coolant (m³), data type `Number`;
...
- `currentFlowCoolant` - current flow of coolant (m³/h), data type `Number`;
- `currentFlowCoolant32` - current flow of coolant (32 bit) (m³/h), data type `Number`;
- `power` - power (kW), data type `Number`;

...
function parseReasonVega (val) {
  ...
  else if ( val === 1 ) return 'byMotionSensorTriggered';
  else return 'notValidValue';
}
function uInt8toInputsVega (val) {
  var result = {};
  var MASK_MOTION_SENSOR_STATE = 0x01;
  var motionSensorState = ( ( val & MASK_MOTION_SENSOR_STATE ) >> 0 );
  if ( motionSensorState === 0x01 ) result.moving = true;
  else  result.moving = false;
  return result;
}
...
function Decode (fPort, bytes, variables) {
    ...
    if ( fPort === 2 )
    {
      ...
      if ( type === 1 )
      {
        ...
        result.angle = readUInt8(bytes,2);

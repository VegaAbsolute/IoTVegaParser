...
function uInt8toCurrentSettingsVegaMS (val) {
  var MASK_TYPE_ACTIVATION      = 0x01;
  var MASK_CONFIRMATION_REQUEST = 0x02;
  var MASK_CONNECTION_PERIOD    = 0x10;
  var typeActivation 	= ( ( val & MASK_TYPE_ACTIVATION   ) >> 0 );
  var confirmationRequest = ( ( val & MASK_CONFIRMATION_REQUEST   ) >> 1 );
  ...
  return {
    ...
    confirmationRequest: confirmationRequest?'enable':'disable',
    connectPeriodMin:  connectPeriod
  };
}
function parseReasonVega (val) {
  if ( val === 0 ) return 'byTime';
  else if( val === 1 ) return 'byAlarm';
  else if( val === 2 ) return 'byAutoArming';
...
function decodeUplink (input) {
  ...
  if ( fPort === 2 )
  {
    ...
    if ( type === 1 )
    {
      ...
      var currentSettings = uInt8toCurrentSettingsVegaMS(cs);
      ...
      result.time = readUInt32LE(bytes,6);

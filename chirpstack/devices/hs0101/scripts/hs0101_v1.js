...
function parseOpeningVega (val) {
  if ( val === 0 ) return 'open';
  else if( val === 1 ) return 'close';
...
function Decode (fPort, bytes, variables) {
    var result = {
        decoder:"vega_hs_0101_v1",
        statusDecode: false
      };
      if ( fPort === 2 )
      {
        var type = readUInt8(bytes,0);
        if ( type === 1 ) result.type = 'currentValues';
        else if ( type === 2 ) result.type = 'byOpeningSensor1Triggered';
        else if ( type === 3 ) result.type = 'byOpeningSensor2Triggered';
        else if ( type === 4 ) result.type = 'byMotionSensorTriggered';
        else if ( type === 5 ) result.type = 'byTemperatureAlarm';
        else if ( type === 6 ) result.type = 'byHumidityAlarm';
        if ( result.type !== undefined ) 
        {
          var rawOpeningSensor1 = readUInt8(bytes,9);
          var rawOpeningSensor2 = readUInt8(bytes,10);
          var openingSensor1 = parseOpeningVega(rawOpeningSensor1);
          var openingSensor2 = parseOpeningVega(rawOpeningSensor2);
          ...
          result.temperature = readInt16LE(bytes,6)/10;
          result.humidityPercent = readUInt8(bytes,8);
          result.openingSensor1 = openingSensor1;
          result.openingSensor2 = openingSensor2;
          result.angle = readUInt8(bytes,11);
          result.lowHumidityPercent = readUInt8(bytes,12);
          result.highHumidityPercent = readUInt8(bytes,13);
          result.lowTemperature = readInt8(bytes,14);
          result.highTemperature = readInt8(bytes,15);

...
function parseReasonVega (val) {
  ...
  else if ( val === 3 ) return 'bySecurityInput3Triggered';
  else if ( val === 4 ) return 'bySecurityInput4Triggered';
  else if ( val === 5 ) return 'byTemperatureOutOfRange';
}
function parseTemperatureOutOfRange (val) {
...
function Decode (fPort, bytes, variables) {
  ...
  if ( fPort === 2 )
  {
    ...
    var rawTemperatureOutOfRange = readUInt8(bytes,24);
    var reason = parseReasonVega(rawReason);
    var isTemperatureOutOfRange = parseTemperatureOutOfRange(rawTemperatureOutOfRange);
    ...
    result.input1State = readUInt32LE(bytes,8);
    result.input2State = readUInt32LE(bytes,12);
    result.input3State = readUInt32LE(bytes,16);
    result.input4State = readUInt32LE(bytes,20);
    result.isTemperatureOutOfRange = isTemperatureOutOfRange;
    result.lowTemperature = readInt8(bytes,25);
    result.highTemperature = readInt8(bytes,26);

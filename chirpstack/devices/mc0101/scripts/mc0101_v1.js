...
function parseReasonVega (val) {
  ...
  else if ( val === 1 ) return 'byOpenSensor1Triggered';
  else if ( val === 2 ) return 'byOpenSensor2Triggered';
  else return 'notValidValue';
}
function uInt8toInputsVega (val) {
  var result = {};
  var MASK_OPEN_SENSOR_1_STATE = 0x01;
  var MASK_OPEN_SENSOR_2_STATE = 0x02;
  var openSensor1State = ( ( val & MASK_OPEN_SENSOR_1_STATE ) >> 0 );
  var openSensor2State = ( ( val & MASK_OPEN_SENSOR_2_STATE ) >> 1 );
  if ( openSensor1State === 0x01 ) result.openSensor1State = 'close';
  else  result.openSensor1State = 'open';
  if ( openSensor2State === 0x01 ) result.openSensor2State = 'close';
  else  result.openSensor2State = 'open';
  return result;
}
...
function decodeUplink (input) {
  ...
  if ( fPort === 2 )
  {
    ...
    if ( type === 1 )
    {
      var rawReason = readUInt8(bytes,5);
      var rawInputs = readUInt8(bytes,6);
      ...
      result.temperature = readInt16LE(bytes,3)/10;

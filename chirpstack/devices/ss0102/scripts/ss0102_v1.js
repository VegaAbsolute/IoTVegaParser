...
function parseType (val) {
  if      ( val === 5 )  return 'fireDetectionAlarm';
  else if ( val === 6 )  return 'test';
  else if ( val === 7 )  return 'solidarityWorkLineAlarm';
  else if ( val === 8 )  return 'removalFromMountingPlatform';
  else if ( val === 9 )  return 'AlarmReset';
  else if ( val === 10 ) return 'lowBatteryCharge';
  else if ( val === 11 ) return 'startOfDeviceGuard';
  else if ( val === 12 ) return 'stopOfDeviceGuard';
  else if ( val === 14 ) return 'sensorMalfunction';
  else if ( val === 15 ) return 'dustinessOfSensorChamber';
  else if ( val === 16 ) return 'bySchedule';
  else return 'notValidValue';
}
function uInt8toCurrentState (val) {
  var result = {};
  var MASK_IS_DUSTINESS_DETECTED               = 0x01;
  var MASK_IS_FIRE_DETECTED                    = 0x02;
  var MASK_IS_TEST_ENABLED                     = 0x04;
  var MASK_IS_STANDBY_ENABLED                  = 0x08;
  var MASK_IS_MALFUNCTION_DETECTED             = 0x10;
  var MASK_IS_ALARM                            = 0x20;
  var MASK_IS_REMOVALED_FROM_MOUNTING_PLATFORM = 0x40;
  var MASK_IS_SOLIDARITY_WORK_LINE_SIGNAL      = 0x80;
  var isDustinessDetected             = ( ( val & MASK_IS_DUSTINESS_DETECTED )               >> 0 );
  var isFireDetected                  = ( ( val & MASK_IS_FIRE_DETECTED )                    >> 1 );
  var isTestEnabled                   = ( ( val & MASK_IS_TEST_ENABLED )                     >> 2 );
  var isStandbyEnabled                = ( ( val & MASK_IS_STANDBY_ENABLED )                  >> 3 );
  var isMalfunctionDetected           = ( ( val & MASK_IS_MALFUNCTION_DETECTED )             >> 4 );
  var isAlarm                         = ( ( val & MASK_IS_ALARM )                            >> 5 );
  var isRemovaledFromMountingPlatform = ( ( val & MASK_IS_REMOVALED_FROM_MOUNTING_PLATFORM ) >> 6 );
  var isSolidarityWorkLineSignal      = ( ( val & MASK_IS_SOLIDARITY_WORK_LINE_SIGNAL )      >> 7 );
  result.isDustinessDetected = isDustinessDetected?true:false;
  result.isFireDetected = isFireDetected?true:false;
  result.isTestEnabled = isTestEnabled?true:false;
  result.isStandbyEnabled = isStandbyEnabled?true:false;
  result.isMalfunctionDetected = isMalfunctionDetected?true:false;
  result.isAlarm = isAlarm?true:false;
  result.isRemovaledFromMountingPlatform = isRemovaledFromMountingPlatform?true:false;
  result.isSolidarityWorkLineSignal = isSolidarityWorkLineSignal?true:false;
  return result;
}
...
function Decode (fPort, bytes, variables) {
  var result = {
    decoder:"vega_ss0102_v1",
    statusDecode: false
  };
  if ( fPort === 2 )
  {
    var types = [5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16];
    var rawType = readUInt8(bytes,0);

    if ( types.indexOf(rawType) >= 0 )
    {
      var rawCurrentState = readUInt8(bytes,5);

      result.type = parseType(rawType);
      result.time = readUInt32LE(bytes,1);
      result.timeStringISO = new Date(result.time*1000).toISOString();
      result.currentState = uInt8toCurrentState(rawCurrentState);
      result.rxVoltage = readUInt16LE(bytes,6);
      result.txCurrent = readUInt16LE(bytes,8);
      result.temperature = readInt16LE(bytes,10);
      result.isBattery1Used = readUInt8(bytes,12)?true:false;
      result.isBattery2Used = readUInt8(bytes,13)?true:false;
      result.isBattery1Connected = readUInt8(bytes,14)?true:false;
      result.isBattery2Connected = readUInt8(bytes,15)?true:false;
      result.battery1ChargePercent = readUInt8(bytes,16);
      result.battery2ChargePercent = readUInt8(bytes,17);

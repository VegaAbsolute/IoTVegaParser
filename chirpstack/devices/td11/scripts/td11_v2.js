...
function parseReasonVega (val) {
  ...
  else if ( val === 3 ) return 'byTemperatureOutOfRange';
  else if ( val === 4 ) return 'byPulseInputOutOfRange';
  else if ( val === 5 ) return 'byServerRequest';
  else return 'notValidValue';
}
function uInt8toInputsVega (val) {
  ...
  var inputType = ( ( val & MASK_INPUT_TYPE ) >> 4 );
  if ( securityInputState === 0x00 ) result.securityInputState = 'short';
  else if ( securityInputState === 0x01 ) result.securityInputState = 'open';
  ...
  if(inputType === 0) result.inputType = 'guard';
  else if(inputType === 1) result.inputType = 'pulse'
  return result;
}
function parseSettingVega (setting)
{
  ...
  else if ( setting.id === 49 )
  {
    ...
    else if ( val === 8 ) result.value = 240;
    else if ( val === 9 ) result.value = 0.25;
    else if ( val === 10 ) result.value = 0.5;
    else if ( val === 11 ) result.value = 1;
    else if ( val === 12 ) result.value = 2;
    else if ( val === 13 ) result.value = 3;
    else if ( val === 14 ) result.value = 4;
    else if ( val === 15 ) result.value = 180;
    else if ( val === 16 ) result.value = 540;
...
function decodeUplink (input) {
  ...
  if ( fPort === 2 )
  {
    ...
    if ( type >= 0 && type <= 5 )
    {
      ...
      result.reason = parseReasonVega(type);
      result.chargePercent = readUInt8(bytes,1);
      var rawTemperatureOutOfRange = readUInt8(bytes,2);
      result.isTemperatureOutOfRange = parseTemperatureOutOfRangeVega(rawTemperatureOutOfRange);
      result.time = readUInt32LE(bytes,3);
      result.timeStringISO = new Date(result.time*1000).toISOString();
      result.deviceTemperature = readUInt8(bytes,7);
      result.probeTemperature = readInt16LE(bytes,8)/10;
      result.lowTemperature = readInt8(bytes,10);
      result.highTemperature = readInt8(bytes,11);
      result.input = readUInt32LE(bytes,12);
      var rawInputs = readUInt8(bytes,16);
      result.inputs = uInt8toInputsVega(rawInputs);

...
function parseTypeVega (val) {
  if ( val === 10 ) return 'standartPacket';
  else if ( val === 11 ) return 'timestampPacket';
  else if ( val === 12 ) return 'oneWirePacket';
  else if ( val === 13 ) return 'modbusPacket';
...
function parseReasonVega (val) {
  ...
  else if ( val === 3 ) return 'bySecurityInput2Triggered';
  else if ( val === 4 ) return 'byConfiguratorRequest';
  else if ( val === 5 ) return 'byButtonTriggered';
  else return 'notValidValue';
}
function int16toTemperatureSensorData (val) {
  var MASK_TEMPERATURE = 0x7FF;
  var MASK_NUMBER      = 0x1F00;
  var rawTemperature = ( ( val & MASK_TEMPERATURE   ) >> 0 );
  var number = ( ( val & MASK_NUMBER   ) >> 11 );
  var temperature = (rawTemperature - 600) / 10;
  return {
    number: number,
    temperature: temperature,
  };
}
function uInt8toModbusHead (val) {
  var MASK_LENGTH = 0x0F;
  var MASK_FUNCTION_CODE = 0xF0;
  var length = ( ( val & MASK_LENGTH   ) >> 0 );
  var functionNumber = ( ( val & MASK_FUNCTION_CODE   ) >> 4 );
  return {
    length: length,
...
function parsePacket(bytes) {
  ...
  var type = parseTypeVega(rawType);
  
  if ( type === 'standartPacket' )
  {
    var rawReason = readUInt8(bytes,1);
    ...
    result.chargePercent = readUInt8(bytes,6);
    result.temperature = readUInt8(bytes,7);
    result.pulseInput1State = readUInt32LE(bytes,8);
    result.pulseInput2State = readUInt32LE(bytes,12);
    result.voltageInput1State = readUInt16LE(bytes,16);
    result.voltageInput2State = readUInt16LE(bytes,18);
  }
  if ( type === 'timestampPacket' )
  {
    var rawReason = readUInt8(bytes,1);
    var reason = parseReasonVega(rawReason);
  ...
  if ( type === 'oneWirePacket' )
  {
    result.type = type;
    result.length = readUInt8(bytes,1);
    result.temperatureSensors = {};
    var rawTemperatureSensors = bytes.slice(2);
    while( rawTemperatureSensors.length > 0 )
    {
      var rawValue = readInt16LE(bytes,0);
      var value = int16toTemperatureSensorData(rawValue);
      if(value.number !== undefined) 
      {
        result.temperatureSensors[value.number] = {
          number: value.number,
          temperature: value.temperature,
        };
      }
      rawTemperatureSensors = rawTemperatureSensors.slice(2);
    }
  }
  if ( type === 'modbusPacket' )
  {
    result.type = type;
    result.length = readUInt8(bytes,1);
    result.modbusData = {};
    var rawModbusData = bytes.slice(2);
    var count = 1;
    while( rawModbusData.length > 0 )
    {
      result.modbusData[count] = {};
      var rawHead = readUInt8(rawModbusData,0);
      var head = uInt8toModbusHead(rawHead);
      var length = head.length;
      var data = rawModbusData.slice(1,length+1).reverse();
      if ( length === 1 )
      {
        result.modbusData[count].value = readUInt8(data,0);
      }
      if ( length === 2 )
      {
        result.modbusData[count].value = readInt16LE(data,0);
      }
      result.modbusData[count].head = head;
      result.modbusData[count].data = data;
      count++;
      rawModbusData = rawModbusData.slice(1+length);
    }
  }
  return result;
}
function parseTextInfo(bytes) {
  var result = [];
  for ( var i = 0; i < 16; i++ )
  {
    var code = readUInt8(bytes,i);
...
function Decode (fPort, bytes, variables) {
    ...
    if ( fPort === 2 )
    {
      while( bytes.length > 0 )
      {
        ...
        if ( packet.type === 'standartPacket' )
        {
          result.standartPacket = packet;
          bytes = bytes.slice(20);
        }
        else if ( packet.type === 'timestampPacket' )
        {
          result.timestampPacket = packet;
          bytes = bytes.slice(6);
        }
        else if ( packet.type === 'oneWirePacket' )
        {
          result.oneWirePacket = packet;
          bytes = bytes.slice(2+packet.length);
        }
        else if ( packet.type === 'modbusPacket' )
        {
          result.modbusPacket = packet;
          bytes = bytes.slice(2+packet.length);
        }
        else
        {
    ...
    else if ( fPort === 195 )
    {
      ...
      if ( type === 195 )
      {
        var reason = readUInt8(bytes,1)?'byRequest':'byDefault';

        result.type = 'infoPacket';
        result.reason = reason;
        result.vendor = parseTextInfo(bytes.slice(2,18));
        result.model = parseTextInfo(bytes.slice(18,34));
        result.mdf = readUInt32LE(bytes,34);
        result.mdfStringISO = new Date(result.mdf*1000).toISOString();
        result.hwVer = readUInt16LE(bytes,38);
        result.swVer = readUInt16LE(bytes,40);
        result.protocolVer = readUInt16LE(bytes,42);
        result.chargePercent = readUInt8(bytes,44);
        result.packetCounter = readUInt32LE(bytes,45);
        result.statusDecode = true;
      }
    }
    return result;

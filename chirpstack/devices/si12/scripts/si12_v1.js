...
function uInt8toCurrentSettingsVegaSI12 (val) {
  var MASK_TYPE_ACTIVATION      = 0x01;
  var MASK_CONNECTION_PERIOD    = 0x0E;
  var MASK_INPUT_1_MODE         = 0x10;
  var MASK_INPUT_2_MODE         = 0x20;
  var MASK_INPUT_3_MODE         = 0x40;
  var MASK_INPUT_4_MODE         = 0x80;
  var typeActivation 	= ( ( val & MASK_TYPE_ACTIVATION   ) >> 0 );
	var connectPeriod  	= ( ( val & MASK_CONNECTION_PERIOD ) >> 1 );
	var input1Mode    	= ( ( val & MASK_INPUT_1_MODE )      >> 4 );
	var input2Mode    	= ( ( val & MASK_INPUT_2_MODE )      >> 5 );
	var input3Mode    	= ( ( val & MASK_INPUT_3_MODE )      >> 6 );
	var input4Mode    	= ( ( val & MASK_INPUT_4_MODE )      >> 7 );
  if      ( connectPeriod === 0x00 ) connectPeriod = 5; 
  else if ( connectPeriod === 0x01 ) connectPeriod = 15; 
  else if ( connectPeriod === 0x02 ) connectPeriod = 30; 
  else if ( connectPeriod === 0x03 ) connectPeriod = 60; 
  else if ( connectPeriod === 0x04 ) connectPeriod = 360; 
  else if ( connectPeriod === 0x05 ) connectPeriod = 720; 
  else if ( connectPeriod === 0x06 ) connectPeriod = 1440;
  return {
    typeActivation: typeActivation?'ABP':'OTAA',
    connectPeriodMin:  connectPeriod,
    input1Mode: input1Mode?'guard':'pulse',
    input2Mode: input2Mode?'guard':'pulse',
    input3Mode: input3Mode?'guard':'pulse',
    input4Mode: input4Mode?'guard':'pulse',
  };
}
function parsePowerStateVega (val) {
  if ( val === 0 ) return 'Off';
  else if ( val === 1 ) return 'On';
  else return 'notValidValue';
}
function uInt8toInputsModeVegaSI12 (val) {
  var MASK_INPUT_1_MODE         = 0x01;
  var MASK_INPUT_2_MODE         = 0x02;
  var MASK_INPUT_3_MODE         = 0x04;
  var MASK_INPUT_4_MODE         = 0x08;
	var input1Mode    	= ( ( val & MASK_INPUT_1_MODE )      >> 0 );
	var input2Mode    	= ( ( val & MASK_INPUT_2_MODE )      >> 1 );
	var input3Mode    	= ( ( val & MASK_INPUT_3_MODE )      >> 2 );
	var input4Mode    	= ( ( val & MASK_INPUT_4_MODE )      >> 3 );
  return {
    input1Mode: input1Mode?'guard':'pulse',
    input2Mode: input2Mode?'guard':'pulse',
    input3Mode: input3Mode?'guard':'pulse',
...
function uInt8toOutputsStateVegaSI12 (val) {
  var MASK_OUTPUT_1_STATE         = 0x01;
  var MASK_OUTPUT_2_STATE         = 0x02;
	var output1State    	= ( ( val & MASK_OUTPUT_1_STATE )      >> 0 );
	var output2State    	= ( ( val & MASK_OUTPUT_2_STATE )      >> 1 );
  return {
    output1State: output1State?'closure':'unlocking',
    output2State: output2State?'closure':'unlocking',
  };
}
...
function decodeUplink (input) {
  ...
  if ( fPort === 2 )
  {
    ...
    if ( type === 1 )
    {
      ...
      var currentSettings = uInt8toCurrentSettingsVegaSI12(cs);
      ...
      if ( currentSettings.input1Mode === 'guard' ) result.input1 = result.input1 ? 'closure' : 'unlocking';
      if ( currentSettings.input2Mode === 'guard' ) result.input2 = result.input2 ? 'closure' : 'unlocking';
      if ( currentSettings.input3Mode === 'guard' ) result.input3 = result.input3 ? 'closure' : 'unlocking';
      if ( currentSettings.input4Mode === 'guard' ) result.input4 = result.input4 ? 'closure' : 'unlocking';
    ...
    else if ( type === 2 )
    {
      ...
      result.charge = readUInt8(bytes,1);
	    result.currentSettings = currentSettings;
      result.dangerInputNumber = readUInt8(bytes,3);
    ...
    {
      var cs = readUInt8(bytes,2);
      var rawPowerState = readUInt8(bytes,3);
      var currentSettings = uInt8toCurrentSettingsVegaSI12(cs);
      var powerState = parsePowerStateVega(rawPowerState);
      
      result.type = 'powerStateChanged';
      result.charge = readUInt8(bytes,1);
	    result.currentSettings = currentSettings;
    ...
    else if ( type === 5 )
    {
      ...
      result.charge = readUInt8(bytes,1);
	    result.currentSettings = currentSettings;
      result.changedOutputNumber = readUInt8(bytes,3);
      result.changedOutputState = readUInt8(bytes,4);
    ...
    {
      var rawInputsMode = readUInt16LE(bytes,5);
      var rawOutputsState = readUInt16LE(bytes,7);
      var inputsMode = uInt8toInputsModeVegaSI12(rawInputsMode);
      var outputsState = uInt8toOutputsStateVegaSI12(rawOutputsState);
      var rawPowerState = readUInt8(bytes,25);
      var powerState = parsePowerStateVega(rawPowerState);

      result.type = 'inputsStateChanged';
      result.time = readUInt32LE(bytes,1);
      result.timeStringISO = new Date(result.time*1000).toISOString();
      result.inputsMode = inputsMode;
      result.outputsState = outputsState;
      result.input1State = readUInt32LE(bytes,9);
      result.input2State = readUInt32LE(bytes,13);
      result.input3State = readUInt32LE(bytes,17);
      result.input4State = readUInt32LE(bytes,21);
      result.powerState = powerState;
      result.statusDecode = true;
    }
  }

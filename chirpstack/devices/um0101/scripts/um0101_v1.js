...
function parseReasonUM0101 (reason)
{
  if ( reason === 1 ) return 'currentState';
  else if ( reason === 2 ) return 'outRangeCO2';
  else if ( reason === 3 ) return 'outRangeLight';
  else if ( reason === 4 ) return 'accelerometer';
  else if ( reason === 5 ) return 'outRangeHumidity';
  else if ( reason === 6 ) return 'outRangeTemperature';
  else if ( reason === 7 ) return 'outRangeNoise';
...
function decodeUplink (input) {
  ...
  if ( fPort === 2 )
  {
    ...
    if ( type === 1 )
    {
      ...
      var reason = readUInt8(bytes,0);
      result.reason = parseReasonUM0101(reason);
      ...
      var powerType = readUInt8(bytes,6);
      if ( powerType === 1 ) result.powerType = 'batteries';
      else if ( powerType === 2 ) result.powerType = 'external';
      else result.powerType = 'notValidValue';
      result.temperature = readInt16LE(bytes,7)/10;
      result.humidityPercent = readUInt8(bytes,9);
      result.lightLux = readUInt16LE(bytes,10);
      result.noisedB = readUInt8(bytes,9);
      result.CO2ppm = readUInt16LE(bytes,13);
      result.angle = readUInt8(bytes,15);
      result.lowTemperature = readInt8(bytes,16);
      result.highTemperature = readInt8(bytes,17);
      result.lowHumidityPercent = readUInt8(bytes,18);
      result.highHumidityPercent = readUInt8(bytes,19);
      result.lowLightLux = readUInt16LE(bytes,20);
      result.highLightLux = readUInt16LE(bytes,22);
      result.lowNoisedB = readUInt8(bytes,24);
      result.highNoisedB = readUInt8(bytes,25);
      result.lowCO2ppm = readUInt8(bytes,26)*10;
      result.highCO2ppm = readUInt8(bytes,27)*10;
